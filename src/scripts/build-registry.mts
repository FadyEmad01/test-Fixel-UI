...
function replaceRegistryPaths(inputStr: string): string {
  return inputStr.replace(/(['"])([\s\S]*?)\1/g, (match, quote, content) => {
    // Handle @/registry/ paths (from monorepo structure)
    if (content.startsWith('@/registry/')) {
      ...
      // Default to components/www/animate-ui for animate-ui components
      if (rest.startsWith('animate-ui/')) {
        return `${quote}@/components/www/animate-ui/${rest.slice('animate-ui/'.length)}${quote}`;
      }
      // For other registry paths, map to components
 ...
 * It searches for all registry-item.json files in the registry directory (if exists),
 * removes the $schema property, and merges them into the base registry.json items array.
 * If no registry folder exists, it uses the existing registry.json items.
 */
async function buildRegistryFile() {
  ...
  let newItems: any[] = [];
  try {
    await fs.access(registryFolderPath);
    newItems = await getRegistryItemsFromFolder(registryFolderPath);
  } catch {
    // Registry folder doesn't exist, use existing items from registry.json
    console.log('No registry folder found, using existing registry.json items');
    newItems = registryData.items || [];
  }
...
export const index: Record<string, any> = {`;
  ...
  for (const item of uniqueItemsMap.values()) {
    ...
    // Convert src/components/... to @/components/... for the import path
    let componentPath = '';
    if (item.files[0]?.path) {
      const filePath = item.files[0].path;
      if (filePath.startsWith('src/')) {
        componentPath = `@/${filePath.slice(4)}`; // Remove 'src/' prefix
      } else {
  ...
  const registryIndexPath = path.join(process.cwd(), '__registry__', 'index.tsx');
...
async function buildRegistry() {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      // Replace `@/registry` paths in file contents
      if (registryItem.files) {
        registryItem.files = registryItem.files.map((file: any) => {
          if (file.content && file.content.includes('@/registry')) {
